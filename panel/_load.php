...
if (isset($_GET['pg'])) {
	...
	if ($pg == 'waktu') {
		...
		while ($log = mysqli_fetch_array($logQ)) {
			...
			if ($log['type'] == 'login' || $log['type'] == 'logout') {
				...
				($log['type'] == 'login') ? $gbr = '../dist/img/login.png' : $gbr = '../dist/img/exit1.svg';
				echo "
						<div class='direct-chat-msg' >
							...
							<span class='direct-chat-img'><img src='$gbr' width=45></span><!-- /.direct-chat-img -->
				...
				echo "
						<div class='direct-chat-msg right'>
							...
							<span class='direct-chat-img'><img src='../dist/img/reading.svg' width=45></span><!-- /.direct-chat-img -->
							<div class='direct-chat-text'>
	...
	elseif ($pg == 'token') {
		...
		if ($cek <> 0) {
			...
			$query2 = mysqli_fetch_array(mysqli_query($koneksi, "SELECT catat_login from setting"));

				if($query2['catat_login']==2){ //jika status reset login di buat automatis (2)
					//cari siswa yang sedang ujian
					$query3 = mysqli_query($koneksi, "SELECT id_siswa from nilai where online=1 and selesai=0");
					foreach ($query3 as $value) {
						$kode= $value['id_siswa'];
						$query4 = mysqli_query($koneksi, "UPDATE nilai set online=0, selesai=0 where id_siswa='$kode'");
						
					}
					$exec1 = mysqli_query($koneksi, "TRUNCATE login"); //bersih tabel login
					$exec = mysqli_query($koneksi, "UPDATE token set token='$token', time='$now' where  id_token='1'"); //simpan token baru
