        ...
        <div class='box box-solid'>
            <div class='box-header with-border '>
                ...
                <div class='box-tools pull-right'>
                    ...
                    <button id='btnstatus' class='btn btn-sm btn-primary'><i class="fas fa-lock-open"></i> Open</button>
                    <button id='btnstatus2' class='btn btn-sm btn-danger'><i class="fas fa-lock"></i> Block </button>
                     <?php } ?>
                     <?php if($_SESSION['level']=="guru"){ ?>
                    <button id='btnstatus' class='btn btn-sm btn-primary'><i class="fas fa-lock-open"></i> Aktifkan</button>
                    <button id='btnstatus2' class='btn btn-sm btn-danger'><i class="fas fa-lock"></i> Non Aktifkan</button>
                    <?php } ?>
                </div>
            </div><!-- /.box-header -->
            <div class='box-body'>
                <div class='table-responsive'>
                    <table style="font-size: 11px" id='tablesiswa' class='table table-bordered table-striped'>
                        ...
                        <tbody>
                            <?php $no = 1; ?>
                            <?php if($_SESSION['level']=="admin"){ ?>
                            <?php $siswa = mysqli_query($koneksi, "SELECT * FROM siswa ORDER BY nama ASC") ?>
                            <?php while ($data = mysqli_fetch_array($siswa)) { ?>
                                <tr>
                                    ...
                                    <td>
                                        <?php if ($data['status_siswa'] ==1) { ?>
                                            <span class="badge bg-green">Active</span>
                                        <?php  } else { ?>
                                            <span class="badge bg-red">Locked</span>
                                        <?php } ?>
                                    </td>
                                </tr>
                            <?php } ?>
