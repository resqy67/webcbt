        ...
        if ($data <> '') {
            foreach ($data as $table) {
                if ($table <> 'pengawas') {
                    ...
                    $a = mysqli_query($koneksi, "DELETE FROM $table");
                    $a = mysqli_query($koneksi, "ALTER TABLE $table AUTO_INCREMENT =1");
                } else {
                    $a = mysqli_query($koneksi, "DELETE FROM $table WHERE level !='admin'");
                     $a = mysqli_query($koneksi, "ALTER TABLE $table AUTO_INCREMENT =1");
                }
            }
            echo "ok";
