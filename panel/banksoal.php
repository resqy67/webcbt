...
if ($ac == '') :
	...
	if (isset($_POST['editbanksoal'])) :
		...
		if ($pengawas['level'] == 'admin') {
			$exec = mysqli_query($koneksi, "UPDATE mapel SET idpk='$idpk',nama='$kode_bank',level='$level',jml_soal='$jml_soal',jml_esai='$jml_esai',status='$status',idguru='$guru',bobot_pg='$bobot_pg',bobot_esai='$bobot_esai',tampil_pg='$tampil_pg',tampil_esai='$tampil_esai',kelas='$kelas',opsi='$opsi',siswa='$siswa' WHERE id_mapel='$id'");
			(!$exec) ? $info = info("Gagal menyimpan!", "NO") : jump("?pg=$pg");
		} elseif ($pengawas['level'] == 'guru') {
			$exec = mysqli_query($koneksi, "UPDATE mapel SET idpk='$idpk',nama='$kode_bank',level='$level',jml_soal='$jml_soal',jml_esai='$jml_esai',status='$status',bobot_pg='$bobot_pg',bobot_esai='$bobot_esai',tampil_pg='$tampil_pg',tampil_esai='$tampil_esai',kelas='$kelas',opsi='$opsi',siswa='$siswa' WHERE id_mapel='$id'");
			(!$exec) ? $info = info("Gagal menyimpan!", "NO") : jump("?pg=$pg");
		}
	endif;
	...
	if (isset($_POST['tambahsoal'])) :
		...
		if ($pengawas['level'] == 'admin') {
			...
			if ($cek > 0) :
				...
				$exec = mysqli_query($koneksi, "INSERT INTO mapel (idpk, nama, jml_soal,jml_esai,level,status,idguru,bobot_pg,bobot_esai,tampil_pg,tampil_esai,kelas,opsi,siswa) VALUES ('$id_pk','$kode_bank','$jml_soal','$jml_esai','$level','$status','$guru','$bobot_pg','$bobot_esai','$tampil_pg','$tampil_esai','$kelas','$opsi','$siswa')");
            ...
            Maaf Kode Mapel - Level - Kelas Sudah ada !
				...
				$exec = mysqli_query($koneksi, "INSERT INTO mapel (idpk, nama, jml_soal,jml_esai,level,status,idguru,bobot_pg,bobot_esai,tampil_pg,tampil_esai,kelas,opsi,siswa) VALUES ('$id_pk','$kode_bank','$jml_soal','$jml_esai','$level','$status','$id_pengawas','$bobot_pg','$bobot_esai','$tampil_pg','$tampil_esai','$kelas','$opsi','$siswa')");
								...
								<?php while ($mapel = mysqli_fetch_array($mapelQ)) : ?>
									...
									<tr>
										...
										<?php if ($setting['server'] == 'pusat') : ?>
											<td style="text-align:center">
												<div class=''>
													<a title="Lihat Soal" href='?pg=<?= $pg ?>&ac=lihat&id=<?= $mapel['id_mapel'] ?>'><button class='btn btn-flat btn-success btn-flat btn-xs'><i class='fa fa-search'></i></button></a>
													<a title="Upload Soal" href='?pg=<?= $pg ?>&ac=importsoal&id=<?= $mapel['id_mapel'] ?>'><button class='btn btn-info btn-flat btn-xs'><i class='fa fa-upload'></i></button></a>
														<button title="duplikat Soal" data-soal="<?= $mapel['id_mapel'] ?>" href="#" class='btn btn-danger btn-flat btn-xs duplikat'><i class='fa fa-copy '></i></button>
														<button title="duplikat Bank Soal" data-soal1="<?= $mapel['id_mapel'] ?>" href="#" class='btn btn-success btn-flat btn-xs duplikat1'><i class='fa fa-file'></i></button>
													<a title="Edit Bank Soal"><button class='btn btn-warning btn-flat btn-xs' data-toggle='modal' data-target='#editbanksoal<?= $mapel['id_mapel'] ?>'><i class='fa fa-edit'></i></button></a>
												</div>
											</td>
												...
												<form action='' method='post'>
													<div class='modal-body'>
														...
														<div class='form-group'>
															...
															<input value="<?= $mapel['nama'] ?>" placeholder="Misal: BINDO10 atau MTK11" type='text' id='kode_bank' name='kode_bank' onkeyup="this.value = this.value.toUpperCase()" class='form-control' required='true' />
														</div>
														<!-- <div class='form-group'>
														...
														<div class='form-group'>
															<div class='row'>
																<div class='col-md-12'>
																	...
																	<span style="color: red;">Jika ujian ini untuk siswa tertentu seperti remedial, silakan pilih siswanya</span>
																</div>
														</div>
													</div>
														...
														<div class='form-group'>
														    <span style="color: red;">Biarkan 0 semua jika tidak ada soal essai</span>
															<div class='row'>
																<div class='col-md-4'>
			...
			<div class='modal-content'>
				...
				<form action='' method='post'>
					<div class='modal-body'>
						<div class='form-group'>
							<label>Kode Bank Soal | <i>Gunkan Huruf Kapital</i> | Buat Kode Seunik Mungkin dan jangan ada Spasi</label>
							<input placeholder="Misal: BINDO10 atau MTK11" type='text' id='kode_bank' name='kode_bank' onkeyup="this.value = this.value.toUpperCase()" class='form-control' required='true' />
						</div>
						<div class='form-group'>
							<label>Mata Pelajaran</label>
							<select class="form-control select2" name='nama' style='width:100%' required='true'>
								<option value=''></option>";
								<?php
								$pkQ = mysqli_query($koneksi, "SELECT * FROM mata_pelajaran ORDER BY nama_mapel ASC");
								while ($pk = mysqli_fetch_array($pkQ)) : ($pk['kode_mapel'] == $mapel['nama']) ? $s = 'selected' : $s = '';
						...
						<div class='form-group'>
							<div class='row'>
								...
								<div class='col-md-3'>
									<label>Opsi Pilih Jawaban</label>
									<select name='opsi' class='form-control'>
										...
										<td style="text-align:justify">
											...
											<table>
												<tr>
													...
													<?php 
													if ($soal['jawaban'] == 'A') {
														echo"Kunci Jawaban : <b>A</b> <i class='fa fa-check text-green'></i>";
													}
													elseif($soal['jawaban'] == 'B'){
														echo"Kunci Jawaban : <b>B</b> <i class='fa fa-check text-green'></i>";
													}
													elseif($soal['jawaban'] == 'C'){
														echo"Kunci Jawaban : <b>C</b> <i class='fa fa-check text-green'></i>";
													}
													elseif($soal['jawaban'] == 'D'){
														echo"Kunci Jawaban : <b>D</b> <i class='fa fa-check text-green'></i>";
													}
													elseif($soal['jawaban'] == 'E'){
														echo"Kunci Jawaban : <b>E</b> <i class='fa fa-check text-green'></i>";
													}
													else{
