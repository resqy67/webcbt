...
if ($koneksipusat) {
    ...
    if ($query['status'] == 'aktif') {
        ...
        if ($data == 'DATA2') {
            ...
            while ($r = mysqli_fetch_array($sqlcek)) {
               ...
               // $cek = mysqli_num_rows($qmapel);
                ...
                if ($cek == 0) {
                  $mapel2 = fetch($koneksipusat, 'mata_pelajaran', array('kode_mapel' => $r['nama']));
                    $exec = mysqli_query($koneksi, "INSERT INTO mata_pelajaran (kode_mapel,nama_mapel) values ('$mapel2[kode_mapel]','$mapel2[nama_mapel]') ");
                }
        ...
        if ($data == 'DATA3') {
            ...
            $homeurl = $setting['url_host'];
            //link file json web server dan id tokenya untuk validasi
            $link = '/soal_json.php?id='.$setting['db_token'];
            //gabung url
            $url2 = $homeurl.$link;
            //ambil file json di web server 
            //$url_json = file_get_contents($url2);
            //ubah file json ke array asosiatif
        ...
        if ($data == 'DATA4') {
            ...
            while ($r = mysqli_fetch_array($sqlcek)) {
                ...
                $sql = mysqli_query($koneksi, "insert into ujian
                        (id_ujian,id_pk,id_guru,id_mapel,nama,jml_soal,jml_esai,bobot_pg,bobot_esai,lama_ujian,tgl_ujian,tgl_selesai,waktu_ujian,level,sesi,acak,token,hasil,kelas,siswa,tampil_pg,tampil_esai,opsi,kode_ujian,kkm,tombol_selsai,acak_opsi,history,status_reset) values            
                        ...
                        '$r[waktu_ujian]','$r[level]','$r[sesi]','$r[acak]','$r[token]','$r[hasil]','$r[kelas]','$r[siswa]','$r[tampil_pg]','$r[tampil_esai]','$r[opsi]','$r[kode_ujian]','$r[kkm]','$r[tombol_selsai]','$r[acak_opsi]','$r[history]','$r[status_reset]')");
       ...
       <?php }
        ...
        if ($data == 'DATA6') {
            ...
            while ($cek = mysqli_fetch_array($cekfile)) {
                ...
                $urls[$i] = $homeurl . $folder . "/files/" . $cek['nama_file'];
                $i++;
            }
