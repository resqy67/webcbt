...
$id = $_GET['id'];
$nilaiq = mysqli_query($koneksi, "SELECT *  FROM nilai  s LEFT JOIN ujian c ON s.id_ujian=c.id_ujian  where c.status='1' and s.id_siswa<>'' and s.id_ujian='$id' GROUP by s.id_nilai DESC");
while ($nilai = mysqli_fetch_array($nilaiq)) {
	...
	if ($nilaiC <> 0) {
		...
		if ($nilai['ujian_mulai'] <> '' and $nilai['ujian_selesai'] <> '') {
			...
			$btn = "
			<button data-id='$nilai[id_siswa]' class='reset btn btn-warning '><i class='fa fa-recycle'></i> Reset Login</button>
